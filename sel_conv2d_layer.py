...
class SelConv2d(nn.Module):
    ...
    def __init__(self, in_size, in_depth, out_depth, full_depth, **kwargs):
        super(SelConv2d, self).__init__()
        self.in_depth = in_depth
        self.out_depth = out_depth
        self.full_depth = full_depth
        self.attention = nn.Linear(in_size, out_depth * full_depth)
    ...
    def forward(self, x, z):
        ...
        map_size = y.size()[-2:]
        w = self.attention(x.view(batch_size, -1))
        ...
        if self.compute_kl_divergence_loss:
            ...
            kl_divergence = F.kl_div(w_sum_avg, self.kl_accumulated_w)
